    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if confirm.exists():
                ...
                print("instance1: {}".format(instance))
            ...
            # else Create New shortened URL
                ...
                print("instance2: {}".format(instance))
                return HttpResponseRedirect(instance.get_absolute_url())
...
def shortener_detail(request, additional_url):
    ...
    * Detail page about shortened url
    ...
    information, created = Information.objects.get_or_create(
        shorten_url=instance)
    qs = HitUpdatedTime.objects.all().filter(
        information=information).order_by("-updated_at")[:5]
...
url_list = ListView.as_view(model=ShortenURL, queryset=ShortenURL.objects.filter(
